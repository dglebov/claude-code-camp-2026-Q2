...
def list_directory(*, path):
    # Ruby's Dir.entries includes "." and ".."; os.listdir does not. The dotfile filter makes
    # the two produce the same list either way.
    return "\n".join(entry for entry in sorted(os.listdir(path)) if not entry.startswith("."))
...
# The reply is the raw provider dict — Claude will answer with stop_reason "tool_use" and
# nothing here dispatches the tool. Normalizing the response and running the loop is step 05.
response = client.call()
print("Raw response:")
# Ruby's JSON.pretty_generate. ensure_ascii=False matters: Ruby emits UTF-8 literally while
# Python would otherwise escape non-ASCII to \uXXXX.
