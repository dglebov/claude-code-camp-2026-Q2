"""Port of `ruby/11_tui/lib/boukensha/backends/base.rb`.
...
class Base:
    ...
    The `reasoning` block is new in step 12. It is what lets the Agent log model thinking without
    knowing which provider produced it — Anthropic sends `thinking`/`redacted_thinking` blocks,
    Gemini flags a part with `thought`, Ollama puts it on `message.thinking`, and OpenAI's
    Responses API returns `reasoning` items with a `summary` array. Each backend maps its own
    shape; nothing downstream needs to care.

    `signature` exists purely so a block can be replayed: Anthropic rejects a thinking block whose
    signature was altered or dropped, while OpenAI does not want reasoning echoed back at all.
    """
