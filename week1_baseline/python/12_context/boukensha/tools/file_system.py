...
def register(registry, *, working_dir):
    ...
    # list_directory and search_files: disabled for now — leftover from when this app was a
    # coding harness; the current player agent has no use for them. Kept in the file, commented,
    # so they can be re-registered when a task needs them. Ruby step 12 does exactly the same,
    # and the two trees must advertise the same tool surface to the model.
    # @registry.tool(
    #     "list_directory",
    #     description=(
