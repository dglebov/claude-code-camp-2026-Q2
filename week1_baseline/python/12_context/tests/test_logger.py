...
def test_response_records_provider_and_model_names(logger):
    """`task` left with the task classes in step 12. provider/model/cost stay — log_viz renders
    them, and dropping them (as upstream did) silently blanks its chips."""
    logger.response(text="x", usage={"input_tokens": 1, "output_tokens": 2}, backend=FakeBackend())

    last = lines(logger)[-1]
    assert "task" not in last
