...
# ---------- step 12: the context readout -------------------------------------


def test_the_colour_class_tracks_context_pressure():
    """Ruby picks a lipgloss colour per render; Textual switches a class. Same thresholds."""
    assert Tui._ctx_class(0) == "-idle"
    assert Tui._ctx_class(69) == "-idle"
    assert Tui._ctx_class(70) == "-warn"
    assert Tui._ctx_class(84) == "-warn"
    assert Tui._ctx_class(85) == "-alert"
    assert Tui._ctx_class(100) == "-alert"


def test_the_readout_reports_the_context_not_the_session_total(repl):
    """The status bar shows window pressure (current/window), not cumulative session spend —
    those are different numbers, and conflating them is the easy mistake."""
    app = Tui(repl)
    repl.context.context_window = 10_000
    repl.context.update_tokens(7_050)

    assert repl.context.usage_pct() == 71  # half-up, matching Ruby
    assert app._ctx_class(repl.context.usage_pct()) == "-warn"
