...
# Step 12 moved the per-turn limits out of tasks.player into their own `agent:` block; the
# provider/model keys stayed where they were.
SETTINGS = {
    "tasks": {
        "player": {
            "provider": "anthropic",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("openai", backends.OpenAI, "gpt-5.4-mini"),
...
def test_the_session_start_snapshot_records_the_effective_config(settings, stub_client, log_path):
    ...
    # `task` left with the task classes; the window the run is bounded by took its place.
    assert "task" not in start
    assert start["context_window"] == 200_000
