...
def test_the_tool_loop_appends_an_assistant_turn_then_a_tool_result(context, registry, spy):
    ...
    # -1 is the final reply, appended since step 08; the tool turns sit before it.
    assistant, result = context.messages[-3], context.messages[-2]
...
def test_preamble_text_becomes_a_plan_event(context, registry, spy):
    """Step 12 splits this in two: the preamble is its own `plan` event carrying no usage, and the
    response event becomes a placeholder that owns the turn's usage figure. Billing must not be
    counted twice."""
    client = StubClient(tool_use(("t1", "look", {}), reasoning="Let me look."), text("done"))

    build(context, registry, client, spy).run()

    assert spy.only("plan")[0]["text"] == "Let me look."
    assert spy.only("response")[0]["text"] == "(tool use — 1 call)"
...
def test_a_raising_tool_is_caught_logged_and_the_loop_continues(context, registry, spy):
    ...
    # The error text is fed back to the model as the tool result. -1 is the final reply,
    # appended since step 08.
    assert context.messages[-2].content == "ERROR: ValueError: kaboom"
...
def test_turn_end_fires_exactly_once_on_every_exit_path(context, registry, spy, third):
    ...
    assert spy.only("turn_end") == [{"reason": "max_iterations", "iterations": 1, "tokens": 0}]
...
def test_turn_end_reports_completed_when_the_model_stops_on_its_own(context, registry, spy):
    ...
    assert spy.only("turn_end") == [{"reason": "completed", "iterations": 1, "tokens": 0}]
...
def test_the_response_event_carries_the_backend(context, registry, spy):
    backend = object()
    client = StubClient(text("done"))
    ...
    # `task` is gone in step 12 — the task classes were deleted. `backend` stays, because the
    # logger derives provider/model/cost from it and log_viz renders those.
    assert "task" not in event
...
def test_an_explicit_max_iterations_is_used(context, registry, spy):
    """Step 12 resolves the bounds at the run/repl seam from Config, so the Agent simply takes
    what it is given — there is no task-settings fallback left inside it."""
    client = StubClient(tool_use(("t1", "look", {})), text("done"))

    build(context, registry, client, spy, max_iterations=3).run()
...
def test_max_output_tokens_is_sent_on_every_counted_call(context, registry, spy):
    client = StubClient(tool_use(("t1", "look", {})), text("done"))

    build(context, registry, client, spy, max_output_tokens=256).run()
...
# ---------- the final reply is persisted (new in step 08) --------------------


def test_run_appends_the_final_text_to_the_context(context, registry, spy):
    """The REPL replays this context every turn, so the reply has to be in it."""
    client = StubClient(text("All done."))

    result = build(context, registry, client, spy).run()

    assert context.messages[-1].role == "assistant"
    assert context.messages[-1].content == result


def test_run_appends_the_final_text_exactly_once(context, registry, spy):
    client = StubClient(text("All done."))

    build(context, registry, client, spy).run()

    assert [m.content for m in context.messages if m.role == "assistant"] == ["All done."]


def test_the_tool_loop_reply_is_appended_after_the_tool_turns(context, registry, spy):
    client = StubClient(tool_use(("t1", "look", {})), text("A corridor."))

    build(context, registry, client, spy).run()

    roles = [m.role for m in context.messages]
    assert roles[-1] == "assistant"
    assert context.messages[-1].content == "A corridor."
    assert "tool_result" in roles


def test_the_wind_down_reply_is_appended(context, registry, spy):
    """The ceiling path returns text too, so it must land in the context as well."""
    client = StubClient(tool_use(("t1", "look", {})))

    result = build(context, registry, client, spy, max_iterations=1).run()
