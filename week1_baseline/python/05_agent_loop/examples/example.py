...
# The tools resolve paths against the iteration root rather than the process CWD, so the example
# behaves the same whether it is launched from bin/ or from this directory.
...
def list_directory(*, path):
    ...
    # the two produce the same list either way. Note the ", " separator — step 04 used "\n".
    return ", ".join(entry for entry in sorted(os.listdir(base_dir / path)) if not entry.startswith("."))
