...
def test_tools_defaults_to_none_and_is_forwarded_verbatim(builder, net):
    """An empty list must reach the builder as an empty list, not be coerced to None — that is
    how Agent's wind-down call disables tools (plan §5.9)."""
    net(ok())

    Client(builder).call()
    assert builder.tools is None
...
# ---------- 401 gets a message that names the fix (new in step 08) -----------


def test_a_401_reports_an_authentication_failure(builder, net, sleeps):
    fake = net(http_error(401))

    with pytest.raises(ApiError) as excinfo:
        Client(builder).call()

    assert fake.count == 1
    assert str(excinfo.value) == "authentication failed (401) — check your API key"


def test_a_401_does_not_leak_the_provider_body(builder, net, sleeps):
    """The generic non-2xx message interpolates the raw body; the 401 one must not."""
    fake = net(http_error(401, body='{"error":{"message":"invalid x-api-key"}}'))
...
def test_other_client_errors_keep_the_generic_message(builder, net, sleeps):
    fake = net(http_error(403, body='{"type":"error"}'))

    with pytest.raises(ApiError) as excinfo:
        Client(builder).call()

    assert fake.count == 1
    assert str(excinfo.value) == 'API request failed after 1 attempt (403): {"type":"error"}'
