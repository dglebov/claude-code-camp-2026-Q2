"""Port of `ruby/10_standard_tool_library/lib/boukensha/registry.rb`.
...
class Registry:
    ...
    def tool(self, name, *, description, parameters=None, required=None):
        ...
        `required` defaults to None, which Tool.required_keys reads as "every parameter" — the
        behaviour every built-in relies on. Pass an explicit list when some are optional, as
        MCP-discovered tools do.

        Note the call form works for dynamically discovered tools, which have no `def` to
        decorate: `registry.tool(name, description=..., ...)(handler)`.
        """

        def decorator(block):
            self._context.register_tool(
                Tool(str(name), description, parameters or {}, block, required)
            )
            return block
    ...
    def registered(self, name):
        """True once a tool of this name exists. Used to detect MCP name collisions."""
        return str(name) in self._context.tools
