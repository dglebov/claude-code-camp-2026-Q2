...
Run all commands from the skill's directory (or pass an absolute path to
the script). All subcommands accept `--session-dir DIR` if you need more
than one independent session; otherwise they share the default one at
